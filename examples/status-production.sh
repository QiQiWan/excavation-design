...
with sqlite3.connect(path, timeout=3.0) as conn:
    ...
    if {'payload_bytes','workspace_bytes'} <= columns:
        rows=conn.execute('SELECT id,name,payload_bytes,workspace_bytes,revision FROM projects ORDER BY payload_bytes DESC LIMIT 10').fetchall()
        for project_id,name,full_bytes,workspace_bytes,revision in rows:
            print(f'  {project_id} R{revision} {name}: full={int(full_bytes or 0)/1048576:.2f} MB workspace={int(workspace_bytes or 0)/1048576:.2f} MB')
    else:
