...
export default function DesignBasisPanel({ project, basis, onSaved }: { project: Project; basis?: DesignBasis; onSaved: (project: Project) => void | Promise<void> }) {
  ...
  async function save() {
    ...
    setSaving(true); setError(undefined); setSavedMessage(undefined);
    try {
      const settings = {
        ...draft,
      ...
      const response = await api.updateProject(project.id, { designSettings: settings } as Partial<Project>);
      // Some legacy/compacted workspace responses can transiently omit the
      // project identity. Re-read the canonical project instead of allowing an
      // empty response to unmount the active workspace.
      const updated = response?.id === project.id ? response : await api.getProject(project.id);
      setDraft({ ...updated.designSettings });
      await onSaved(updated);
      setSavedMessage('设计基准已保存，当前工程、当前步骤和滚动位置保持不变。');
  ...
  return <section className="designBasisPanel">
    ...
    {savedMessage ? <div className="successNotice" role="status">{savedMessage}</div> : null}
