...
export default function ConstructionStageEditor({ project, onChanged }: { project: Project; onChanged?: () => void | Promise<void> }) {
  ...
  async function save() {
    ...
    try {
      ...
      setNotice('施工阶段已锁定为项目工况；原计算结果已失效，请重新运行当前方案。');
      await onChanged?.();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : String(reason));
  ...
  async function reset() {
    ...
    try {
      ...
      setWorkspace(value); setDraft(copyCase(value.case)); setDirty(false);
      setNotice('已恢复推荐施工阶段；原计算结果已失效，请重新计算。');
      await onChanged?.();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : String(reason));
    } finally { setSaving(false); }
  ...
  if (!workspace || !draft) return <section id="construction-stage-editor" className="constructionStageEditor loading"><strong>正在读取施工阶段与外部计算证据…</strong>{error ? <span>{error}</span> : null}</section>;
  ...
  return <section id="construction-stage-editor" className={`constructionStageEditor ${workspace.summary.validationStatus}`}>
    <header>
      <div><span className="sectionKicker">施工阶段数据源</span><h4>开挖、支撑激活、换撑与拆撑</h4><p>阶段输入由设计人员确认；逐阶段内力和位移由计算生成，不能在配筋页手工补录。</p></div>
    ...
    <div className="constructionStageToolbar">
      ...
      <button type="button" className="secondary" disabled={saving} onClick={() => void reset()}>恢复推荐阶段</button>
    ...
    {issues.length ? <details className="constructionStageIssues" open={issues.some((item) => item.severity === 'fail')}><summary>阶段校验：{issues.length} 项</summary>{issues.map((item, index) => <article key={`${item.code}-${item.stageId ?? index}`} className={item.severity}><strong>{item.code}</strong><span>{item.message}</span><em>{item.action}</em></article>)}</details> : <div className="rebarGateMessage pass">施工阶段标高、构件引用和最终开挖覆盖有效。</div>}

    {expanded ? <div className="constructionStageList">
      <label className="constructionCaseName">算例名称<input value={draft.name} onChange={(event) => updateCase({ name: event.target.value })} /></label>
      {draft.stages.map((stage, index) => <details key={stage.id} className="constructionStageCard" open={index === 0}>
        ...
        <div className="constructionStageFields">
          ...
          <label>施工分区<input value={stage.zone ?? ''} onChange={(event) => updateStage(index, { zone: event.target.value })} /></label>
          ...
          <label>阶段超载（kPa）<input type="number" min="0" step="1" value={stage.surcharge} onChange={(event) => updateStage(index, { surcharge: finite(event.target.value, stage.surcharge) })} /></label>
      ...
      <button type="button" className="secondary addConstructionStage" onClick={addStage}>新增施工阶段</button>
    </div> : null}

    <details className="constructionStageInputGuide"><summary>这些资料在哪里补齐</summary><div>{workspace.inputGuide.map((item) => <article key={item.field}><strong>{item.label}</strong><span>{item.location}</span><p>{item.action}</p><footer>{item.provider} · {item.designStageAvailable ? '设计阶段可提供' : '需专项/施工资料'}</footer></article>)}</div></details>
  </section>;
}
