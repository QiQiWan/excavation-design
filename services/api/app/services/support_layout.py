...
def _corner_diagonal_layout(points: list[Point2D], obstacles: list[tuple[ConstructionObstacle, list[Point2D]]]) -> list[SupportLayoutLine]:
    ...
    for idx, curr in enumerate(points):
        ...
        if len_prev < 2.0 * MIN_CORNER_BRACE_LEG_M or len_next < 2.0 * MIN_CORNER_BRACE_LEG_M:
            continue
        # Keep corner braces inside the first 3-6m bay so they do not cross the
        # newly densified main struts.  Larger corner braces should be modeled as
        # explicit node-connected diagonals in the detailed design stage.
        offset_prev = min(max(2.5, short_span * 0.12), 4.5, len_prev * 0.32)
        offset_next = min(max(2.5, short_span * 0.12), 4.5, len_next * 0.32)
...
def make_support_wale_nodes(supports: list[SupportElement], wale_beams: list[BeamElement]) -> list[SupportWaleNode]:
    ...
    for support in supports:
        for side, point, face_code in (("A", support.start, support.start_face_code), ("B", support.end, support.end_face_code)):
