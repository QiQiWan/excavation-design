...
from app.storage.artifact_store import (
    ...
    rehydrate_geological_evidence,
    rehydrate_latest_calculation_evidence,
)
...
class ProjectRepository:
    ...
    def get(self, project_id: str) -> Project | None:
        ...
        # Canonical full loads must never expose a result generated by an older
        # algorithm/rule-set or a different support topology. Apply the bounded
        # in-memory migration unconditionally; persistence still occurs only on
        # the caller's next explicit save, avoiding read-triggered write
        # amplification for large projects.
    ...
    def get_with_latest_calculation(self, project_id: str) -> Project | None:
        ...
        if not data:
            return None
        hydrated = rehydrate_latest_calculation_evidence(data, ProjectArtifactStore())
        project = Project.model_validate(hydrated)
        _migrate_loaded_project(project)
        return project

    def get_for_calculation(self, project_id: str) -> Project | None:
        """Load canonical inputs plus the full geology and latest stage evidence."""
        data = self.store.get(project_id)
        if not data:
            return None
        artifact_store = ProjectArtifactStore()
        hydrated = rehydrate_geological_evidence(data, artifact_store)
        hydrated = rehydrate_latest_calculation_evidence(hydrated, artifact_store)
        project = Project.model_validate(hydrated)
        _migrate_loaded_project(project)
        return project

    def require_for_calculation(self, project_id: str) -> Project:
        project = self.get_for_calculation(project_id)
        if project is None:
            raise HTTPException(status_code=404, detail=f"Project not found: {project_id}")
        return project
