...
def test_workspace_projection_is_bounded_and_returned_without_full_model_load(tmp_path: Path) -> None:
    ...
    # V3.31 externalizes heavy payloads before persisting the core snapshot.
    assert full_info["externalBytes"] > 0
    assert full_info["payloadBytes"] < 100_000
    assert full_info["workspaceBytes"] < 100_000
