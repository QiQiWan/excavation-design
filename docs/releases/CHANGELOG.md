...
- 新增八阶段渐进式设计会话，依次确认设计域、施工约束、围护墙体系、支撑体系、线位搜索、候选预检、施工阶段计算和深化交付。
- 渐进式配置独立持久化并生成配置追踪哈希；上游决策修改只失效相关下游阶段。
- 固定 96 MB API 阈值升级为内存、cgroup、进程 RSS、CPU 负载、磁盘余量和 JSON 放大系数联合决定的自适应资源策略。
- API 采用工作区优先；完整工程只在独立 worker 中按实时资源准入，A/B/C 批量任务可在大项目全量快照不可交互加载时正常提交。
- 重型任务在每次执行前重新评估并发容量，系统高负载或低内存时自动串行。
- 项目压缩增加内存与磁盘双重预检；资源不足时只用 SQLite JSON1 重建工作区，延后完整外部化。
- 候选平面增加专用轻量预览缓存和旧项目 JSON1 回填，切换 A/B/C 不再出现无说明空白。
- 前端增加渐进式设计配置面板、动态资源状态和明确的诊断/加载/缺失预览状态。

# V3.36.0

- 新增六级通用设计资格链：数据工作集、围护几何、坐标/地质、支撑体系、计算证据、正式交付。
- 工作区只读诊断接口改用有上限投影；完整快照超限时，方案与资格面板仍可操作。
- 新增 `storage_compaction` 后台任务，外部化重型数据、重建工作区并写入审计事件。
- 新增坐标一致性审计和人工确认式平移建议，禁止静默修改工程坐标。
- 建立可扩展结构体系目录，覆盖直撑、角撑混合、分区转接、环梁径向撑、中心岛、环桁架、显式框架和分仓施工。
- A/B/C 面板过滤受控阻断诊断卡；计算入口与独立 worker 同时执行资格门禁。
- 支撑候选支持显式 `topologyFamily`，未实现体系不再降级为普通直撑。
- 增加长条形、近方形、凹形和井筒等跨平面回归。
